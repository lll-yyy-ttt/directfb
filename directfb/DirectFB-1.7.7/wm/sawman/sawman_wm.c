             ...
             int           dy )
{
     ...
     old_region.x1 = 0;
     old_region.y1 = 0;
     old_region.x2 = sawwin->bounds.w - 1;
     old_region.y2 = sawwin->bounds.h - 1;

     new_region.x1 = dx;
     new_region.y1 = dy;
     new_region.x2 = new_region.x1 + sawwin->bounds.w - 1;
     new_region.y2 = new_region.y1 + sawwin->bounds.h - 1;

     window->config.bounds.x += dx;
     window->config.bounds.y += dy;
     ...
     if (SAWMAN_VISIBLE_WINDOW( window ) && (sawwin->flags & SWMWF_INSERTED)) {
          if (dfb_region_region_intersect( &new_region, &old_region )) {
               ...
               if (new_region.x2 < old_region.x2) {
                    DFBRegion region = { new_region.x2 + 1, old_region.y1,
               ...
               int           height )
{
     ...
     sawman_adjust_window_bounds( sawwin, &bounds );
                   ...
                   int           height )
{
     ...
     new_region.x1 = x - sawwin->bounds.x;
     new_region.y1 = y - sawwin->bounds.y;
     new_region.x2 = new_region.x1 + bounds.w - 1;
     new_region.y2 = new_region.y1 + bounds.h - 1;
     ...
     clip_region.x1 = 0;
     clip_region.y1 = 0;
     clip_region.x2 = width - 1;
     clip_region.y2 = height - 1;

     if (!dfb_region_region_intersect( &window->config.opaque, &clip_region ))
          window->config.opaque = clip_region;
...
static bool
sawman_window_bounds_pick( SaWMan *sawman,
                           CoreWindowStack *stack,
                           SaWManWindow *sawwin,
                           int              x,
                           int              y,
                           DFBRectangle    *bounds )
{
     ...
     SaWManTier *tier = sawman_tier_by_class( sawman, sawwin->window->config.stacking );
     D_MAGIC_ASSERT( tier, SaWManTier );

     /* Convert to Tier coordinates */
     int tx = (s64) x * (s64) tier->size.w / (s64) sawman->resolution.w;
     int ty = (s64) y * (s64) tier->size.h / (s64) sawman->resolution.h;
...
static bool
sawman_window_client_pick( SaWMan *sawman,
                           CoreWindowStack *stack,
                           SaWManWindow *sawwin,
                           int              x,
                           int              y )
{
     ...
     bounds.x = 0;
     bounds.y = 0;
     bounds.w = sawwin->dst.w;
     bounds.h = sawwin->dst.h;
                     ...
                     const DFBInputEvent *event )
{
     ...
     sawman->move_grabbed = NULL;

     if (sawwin)
     {
          /* to toplevel*/
          dfb_window_raisetotop(sawwin->window);

          const SaWManBorderInit *border = &sawman_config->borders[sawman_window_priority(sawwin)];
          if ( SAWMAN_BORDER_TYPE_THEME == border->type &&
               sawman_window_border( sawwin ) )
          {
               if (sawman_window_client_pick( sawman, stack, sawwin, -1, -1 ))
               {
               ...
               {
                    DFBRectangle bounds;
                    bounds.x = border->theme.max_rect.x + border->theme.max_rect.w + 1;
                    bounds.y = border->theme.top_edge;
                    bounds.w = sawwin->bounds.w - bounds.x - border->theme.right;
                    bounds.h = border->theme.top - border->theme.top_edge;

                    if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                    {
                         sawman->move_grabbed = sawwin;
                    }
                    else if ( sawman->top_grabbed ||
                              sawman->bottom_grabbed ||
                              sawman->left_grabbed ||
                              sawman->right_grabbed )
                    {
                         sawman->move_grabbed = sawwin;
                    }

                    sawman->delta_x_grabbed = stack->cursor.x - sawwin->bounds.x;
                    sawman->delta_y_grabbed = stack->cursor.y - sawwin->bounds.y;
                    if (sawman->right_grabbed)
                    {
                         sawman->delta_x_grabbed = (sawwin->bounds.x + sawwin->bounds.w - 1) - stack->cursor.x;
                    }
                    if (sawman->bottom_grabbed)
                    {
                         sawman->delta_y_grabbed = (sawwin->bounds.y + sawwin->bounds.h - 1) - stack->cursor.y;
                    }
               }
          }
          else
                       ...
                       const DFBInputEvent *event )
{
     ...
     if (sawman->move_grabbed)
     {
          sawwin = sawman->move_grabbed;
     }
     else if (sawman->focused_window && sawman->focused_window->window->config.cursor_flags & DWCF_EXPLICIT)
     {
     ...
     if (sawwin)
     {
               ...
               sawman_window_border( sawwin ) )
          {
               ...
               if (sawwin->close_focused)
               {
                    CoreWindow *window = sawwin->window;
                    SaWManWindowInfo *info = NULL;
                    ...
                    DirectResult ret;
                    ret = sawman_call( sawman, SWMCID_WINDOW_CLOSE, info, sizeof(*info), false );
                    switch (ret)
                    {
                         case DFB_NOIMPL:
                         {
               ...
               else if(sawwin->min_focused)
               {
                    sawwin->min_focused = false;
                    ...
                    sawman_call( sawman, SWMCID_WINDOW_MIN, info, sizeof(*info), false );
               }
               else if(sawwin->max_focused)
               {
                    sawwin->max_focused = false;
                    ...
                    sawman_call( sawman, SWMCID_WINDOW_MAX, info, sizeof(*info), false );
               }
               else
               {
                    send_button_event( sawman, sawwin, stack, event );
...
static void
sawman_update_motion( SaWMan          *sawman,
                     CoreWindowStack *stack,
                     int              x,
                     int              y )
{
     SaWManTier *tier = NULL;
     ...
     if (!sawman->pointer_window) {
          ...
          if (before != after) {
               ...
               if (before) {
                    ...
                    if (sawman_window_border( before )) 
                    {
                         before->close_focused = false;
                         before->min_focused = false;
                         before->max_focused = false;

                         sawman_update_window( sawman, before, NULL, DSFLIP_NONE, SWMUF_UPDATE_BORDER );

                         tier = sawman_tier_by_class( sawman, before->window->config.stacking );
                         D_ASSERT(tier->region != NULL);
                         if (tier->region->config.options & DLOP_STEREO)
                              sawman_update_window( sawman, before, NULL, DSFLIP_NONE, SWMUF_UPDATE_BORDER | SWMUF_RIGHT_EYE );
                    }
               }
               ...
               if (after) {
                    ...
                    if (sawman_window_border( after )) 
                    {
                         sawman_update_window( sawman, after, NULL, DSFLIP_NONE, SWMUF_UPDATE_BORDER );

                         tier = sawman_tier_by_class( sawman, after->window->config.stacking );
                         D_ASSERT(tier->region != NULL);
                         if (tier->region->config.options & DLOP_STEREO)
                              sawman_update_window( sawman, after, NULL, DSFLIP_NONE, SWMUF_UPDATE_BORDER | SWMUF_RIGHT_EYE );
                    }
               }
               ...
               /* recover cursor */
               if (before && before->window->cursor.surface)
               {
                    const SaWManBorderInit *border = &sawman_config->borders[sawman_window_priority(before)];
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type  )
                    {
                         if (!after || (after && !before->window->cursor.surface))
                         {
                              dfb_windowstack_cursor_set_shape( stack, border->theme.cursor, 0, 0 );
                         }
                    }
               }
          }
               ...
               DFBInputDeviceID  device_id )
{
     ...
     static SaWManWindow *sawwin_old = NULL;
     ...
     {
          sawman_update_motion( sawman, stack, cx, cy );
     ...
     if (sawwin) 
     {
               ...
               sawman_window_border( sawwin ) )
          {
               if (move_window)
               {
                    if (cx < border->theme.desktop_insets.l)
                    {
                         cx = border->theme.desktop_insets.l;
                    }
                    else if (cx >= (sawman->resolution.w - border->theme.desktop_insets.r))
                    {
                         cx = sawman->resolution.w - border->theme.desktop_insets.r - 1;
                    }

                    if (cy < border->theme.desktop_insets.t)
                    {
                         cy = border->theme.desktop_insets.t;
                    }
                    else if (cy >= (sawman->resolution.h - border->theme.desktop_insets.b))
                    {
                         cy = sawman->resolution.h - border->theme.desktop_insets.b - 1;
                    }
               }
          }
     }
     ...
     bool update_cursor = sawman->top_grabbed ||
                          sawman->bottom_grabbed ||
                          sawman->left_grabbed ||
                          sawman->right_grabbed;
     ...
     if (!sawman->move_grabbed)
     {
          sawman->top_grabbed = false;
          sawman->bottom_grabbed = false;
          sawman->left_grabbed = false;
          sawman->right_grabbed = false;
     }
     ...
     if (sawwin) 
     {
          ...
          if ( SAWMAN_BORDER_TYPE_THEME == border->type &&
               sawman_window_border( sawwin ) )
          {
               if (!sawman->move_grabbed && !sawwin->is_maximization)
               {
                    int threshold = 15;
                    DFBRectangle bounds;
                    ...
                    do {
                         ...
                         bounds.x = threshold;
                         bounds.y = 0;
                         bounds.w = sawwin->bounds.w - (threshold * 2);
                         bounds.h = border->theme.top_edge;
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->top_grabbed = true;
                              break;
                         }
                         ...
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->left_grabbed = true;
                              sawman->top_grabbed = true;
                              break;
                         }

                         // right top
                         bounds.x = sawwin->bounds.w - threshold;
                         bounds.y = 0;
                         bounds.w = threshold;
                         bounds.h = border->theme.top_edge;
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->right_grabbed = true;
                              sawman->top_grabbed = true;
                              break;
                         }
                         ...
                         // bottom
                         bounds.x = threshold;
                         bounds.y = sawwin->bounds.h - border->theme.bottom;
                         bounds.w = sawwin->bounds.w - (threshold * 2);
                         bounds.h = border->theme.bottom;
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->bottom_grabbed = true;
                              break;
                         }
                         ...
                         bounds.x = 0;
                         bounds.y = threshold;
                         bounds.w = border->theme.left;
                         bounds.h = sawwin->bounds.h - (threshold * 2);
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->left_grabbed = true;
                              break;
                         ...
                         // left bottom
                         bounds.x = 0;
                         bounds.y = sawwin->bounds.h - threshold;
                         bounds.w = border->theme.left;
                         bounds.h = threshold;
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                         ...
                         // right
                         bounds.x = sawwin->bounds.w - border->theme.right;
                         bounds.y = threshold;
                         bounds.w = border->theme.right;
                         bounds.h = sawwin->bounds.h - (threshold * 2);
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, -1, -1, &bounds ))
                         {
                              sawman->right_grabbed = true;
               ...
               if (move_window)
               {
                    ...
                    {
                         int x1 = sawwin->bounds.x;
                         int y1 = sawwin->bounds.y;
                         int x2 = sawwin->bounds.x + sawwin->bounds.w - 1;
                         int y2 = sawwin->bounds.y + sawwin->bounds.h - 1;

                         if ( sawman->top_grabbed ||
                              sawman->bottom_grabbed ||
                              sawman->left_grabbed ||
                              sawman->right_grabbed )
                         {
                              if (sawman->left_grabbed)
                              {
                                   x1 = cx - sawman->delta_x_grabbed;

                                   if (x1 > (x2 - border->theme.min_window_width))
                                   {
                                        x1 = (x2 - border->theme.min_window_width);
                                   }
                              }
                              if (sawman->top_grabbed)
                              {
                                   y1 = cy - sawman->delta_y_grabbed;

                                   if (y1 > (y2 - border->theme.min_window_height))
                                   {
                                        y1 = (y2 - border->theme.min_window_height);
                                   }
                              }
                              if (sawman->right_grabbed)
                              {
                                   x2 = cx + sawman->delta_x_grabbed;

                                   if (x2 < (x1 + border->theme.min_window_width))
                                   {
                                        x2 = (x1 + border->theme.min_window_width);
                                   }
                              }
                              if (sawman->bottom_grabbed)
                              {
                                   y2 = cy + sawman->delta_y_grabbed;

                                   if (y2 < (y1 + border->theme.min_window_height))
                                   {
                                        y2 = (y1 + border->theme.min_window_height);
                                   }
                              }

                              int width = x2 - x1 + 1 - (border->theme.left + border->theme.right);
                              int height = y2 - y1 + 1 - (border->theme.top + border->theme.bottom);

                              if ( x1 != sawwin->bounds.x || 
                                   y1 != sawwin->bounds.y ||
                                   width != sawwin->bounds.w ||
                                   height != sawwin->bounds.h )
                              {
                                   dfb_window_set_bounds(sawwin->window, x1, y1, width, height);
                              }
                         }
                         else
                         {
                              ...
                              if ( x1 != sawwin->bounds.x || 
                                   y1 != sawwin->bounds.y )
                              {
                                   dfb_window_move(sawwin->window, x1, y1, false);
                              }
                         }
                    }
               }
               else
               {
                    ...
                    if ( sawwin->close_focused ||
                         ...
                         sawwin->max_focused )
                    {
                         sawwin->close_focused = false;
                         sawwin->min_focused = false;
                         sawwin->max_focused = false;
                         update_window = true;
                    }

                    DFBRectangle bounds = border->theme.close_rect;
                    if (sawman_window_bounds_pick( sawman, stack, sawwin, cx, cy, &bounds ))
                    {
                         sawwin->close_focused = true;
                    }
                    else
                    {
                         bounds = border->theme.min_rect;
                         if (sawman_window_bounds_pick( sawman, stack, sawwin, cx, cy, &bounds ))
                         {
                              sawwin->min_focused = true;
                         }
                         else
                         {
                              bounds = border->theme.max_rect;
                              if (sawman_window_bounds_pick( sawman, stack, sawwin, cx, cy, &bounds ))
                              {
                                   sawwin->max_focused = true;
                              }
                         }
                    }

                    if ( update_window ||
                         sawwin->close_focused ||
                         sawwin->min_focused ||
                         sawwin->max_focused )
                    {
                         DFBRegion region;
                         ...
                         SaWManTier *tier = sawman_tier_by_class( sawman, sawwin->window->config.stacking );
                         D_ASSERT(tier->region != NULL);
                         bool stereo_layer = tier->region->config.options & DLOP_STEREO;

                         // update top
                         region = DFB_REGION_INIT_FROM_RECTANGLE_VALS(0, 0, sawwin->bounds.w, border->theme.top );
                         dfb_region_translate( &region, sawwin->window->config.z, 0 );
          ...
          bool update_cursor_2 = sawman->top_grabbed ||
                                 sawman->bottom_grabbed ||
                                 sawman->left_grabbed ||
                                 sawman->right_grabbed;

          if (update_cursor != update_cursor_2 || sawwin_old != sawwin)
          {
               sawwin_old = sawwin;

               if ( sawman->left_grabbed && sawman->top_grabbed )
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.lt, border->theme.lt_hot_x, border->theme.lt_hot_y );
               }
               else if ( sawman->left_grabbed && sawman->bottom_grabbed )
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.lb, border->theme.lb_hot_x, border->theme.lb_hot_y );
               }
               else if ( sawman->right_grabbed && sawman->top_grabbed )
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.rt, border->theme.rt_hot_x, border->theme.rt_hot_y );
               }
               else if ( sawman->right_grabbed && sawman->bottom_grabbed )
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.rb, border->theme.rb_hot_x, border->theme.rb_hot_y );
               }
               else if (sawman->top_grabbed)
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.t, border->theme.t_hot_x, border->theme.t_hot_y );
               }
               else if (sawman->bottom_grabbed)
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.b, border->theme.b_hot_x, border->theme.b_hot_y );
               }
               else if (sawman->left_grabbed)
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.l, border->theme.l_hot_x, border->theme.l_hot_y );
               }
               else if (sawman->right_grabbed)
               {
                    if ( SAWMAN_BORDER_TYPE_THEME == border->type && sawman_window_border(sawwin) )
                         dfb_windowstack_cursor_set_shape( stack, border->theme.r, border->theme.r_hot_x, border->theme.r_hot_y );
               }
               else
               {
                    if (sawwin->window->cursor.surface)
                    {
                         dfb_windowstack_cursor_set_shape( stack, sawwin->window->cursor.surface, sawwin->window->cursor.hot_x, sawwin->window->cursor.hot_y );
                    }
                    else
                    {
                         if ( SAWMAN_BORDER_TYPE_THEME == border->type )
                              dfb_windowstack_cursor_set_shape( stack, border->theme.cursor, border->theme.cursor_hot_x, border->theme.cursor_hot_y );
                    }
               }
          }
     }
               ...
               void            *window_data )
{
     ...
     if ( SAWMAN_BORDER_TYPE_THEME == border->type &&
          sawman_window_border( sawwin ) )
     {
          window->config.border_offset_x = border->theme.left;
          window->config.border_offset_y = border->theme.top;
     }
     ...
     sawman_update_current_focus( sawman, sawwin, false );
                      ...
                      CoreWindowConfigFlags   flags )
{
     ...
     *config = *current; /* make sure that all fields in "request" are valid */
     ...
     reconfig->flags =
          ...
          | (flags & CWCF_NAME         ? SWMCF_NAME         : 0)
          | (flags & CWCF_HIDE         ? SWMCF_HIDE         : 0)
     ...
     switch (ret = sawman_call( sawman, SWMCID_WINDOW_RECONFIG, reconfig, sizeof(*reconfig), true )) {
          case DFB_OK: {
               ...
               flags =
                    ...
                    | (f & SWMCF_NAME         ? CWCF_NAME         : 0)
                    | (f & SWMCF_HIDE         ? CWCF_HIDE         : 0)
     ...
     if (flags & CWCF_NAME) 
     {
          if ( strcmp(window->config.name, config->name) ) 
          {
               strcpy(window->config.name, config->name);
               sawman_update_title( sawwin );
          }
     }

     if (flags & CWCF_HIDE) 
     {
          sawman_set_hide( sawman, sawwin, config->hide );
     }
     ...
     if (flags & (CWCF_POSITION | CWCF_SIZE | CWCF_NAME | CWCF_HIDE | CWCF_OPACITY | CWCF_OPTIONS | CWCF_STEREO_DEPTH))
