...
static DFBResult
IDirectFBWindow_SetName( IDirectFBWindow         *thiz,
                         const char *name )
{
     CoreWindowConfig config;
     ...
     if (strlen(name) > (DFB_WINDOW_NAME_MAX_LENGTH-1))
     {
          strncpy(config.name, name, DFB_WINDOW_NAME_MAX_LENGTH-1);
          config.name[DFB_WINDOW_NAME_MAX_LENGTH-1] = '\0';
     }
     else
     {
     ...
     return CoreWindow_SetConfig( data->window, &config, NULL, 0, CWCF_NAME );
}

static DFBResult
IDirectFBWindow_GetName( IDirectFBWindow               *thiz,
                         char                          *buffer,
                         int				len )
{
     CoreWindowConfig config;
     ...
     buffer[0] = '\0';
     int l = strlen(data->window->config.name);
     int n = (len-1) > l ? l : (len-1);
     strncpy(buffer, data->window->config.name, n);
     buffer[n] = '\0';
...
static DFBResult
IDirectFBWindow_SetHide( IDirectFBWindow         *thiz,
                         DFBBoolean               hide )
{
     CoreWindowConfig config;
     ...
     return CoreWindow_SetConfig( data->window, &config, NULL, 0, CWCF_HIDE );
}

static DFBResult
IDirectFBWindow_GetHide( IDirectFBWindow               *thiz,
                         DFBBoolean                    *hide )
{
     CoreWindowConfig config;
     ...
     *hide = data->window->config.hide;

     return DFB_OK;
}

static DFBResult
IDirectFBWindow_ScreenToClient( IDirectFBWindow        *thiz,
                                int                    *x,
                                int                    *y )
{
     CoreWindowConfig config;
     ...
     *x = *x - (data->window->config.bounds.x + data->window->config.border_offset_x);
     *y = *y - (data->window->config.bounds.y + data->window->config.border_offset_y);
                           ...
                           bool             created )
{
     ...
     thiz->SetName = IDirectFBWindow_SetName;
     thiz->GetName = IDirectFBWindow_GetName;
     thiz->SetHide = IDirectFBWindow_SetHide;
     thiz->GetHide = IDirectFBWindow_GetHide;
     thiz->ScreenToClient = IDirectFBWindow_ScreenToClient;
