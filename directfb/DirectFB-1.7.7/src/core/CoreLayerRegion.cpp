...
)
{
    ...
    switch (CoreDFB_CallMode( core_dfb )) {
        ...
        case COREDFB_CALL_INDIRECT: {
            ...
            Core_PushCalling();
            ret = requestor.SetSurface( surface );
            Core_PopCalling();
...
DFBResult
ILayerRegion_Requestor::SetSurface(
                    CoreSurface                               *surface
)
{
    DFBResult           ret = DFB_OK;
    char        args_static[FLUXED_ARGS_BYTES];
    char        return_args_static[FLUXED_ARGS_BYTES];
    CoreLayerRegionSetSurface       *args = (CoreLayerRegionSetSurface*) args_alloc( args_static, sizeof(CoreLayerRegionSetSurface) );
    CoreLayerRegionSetSurfaceReturn *return_args;

    if (!args)
        return (DFBResult) D_OOM();

    return_args = (CoreLayerRegionSetSurfaceReturn*) args_alloc( return_args_static, sizeof(CoreLayerRegionSetSurfaceReturn) );
    ...
    ret = (DFBResult) CoreLayerRegion_Call( obj, FCEF_NONE, CoreLayerRegion_SetSurface, args, sizeof(CoreLayerRegionSetSurface), return_args, sizeof(CoreLayerRegionSetSurfaceReturn), NULL );
    if (ret) {
        D_DERROR( ret, "%s: CoreLayerRegion_Call( CoreLayerRegion_SetSurface ) failed!\n", __FUNCTION__ );
        goto out;
    }

    if (return_args->result) {
        /*D_DERROR( return_args->result, "%s: CoreLayerRegion_SetSurface failed!\n", __FUNCTION__ );*/
        ret = return_args->result;
        goto out;
    }
                                ...
                                unsigned int *ret_length )
{
    ...
    CoreSurface *surface = NULL;
        ...
        case CoreLayerRegion_SetSurface: {
    CoreSurface *surface = NULL;
            D_UNUSED
            CoreLayerRegionSetSurface       *args        = (CoreLayerRegionSetSurface *) ptr;
            CoreLayerRegionSetSurfaceReturn *return_args = (CoreLayerRegionSetSurfaceReturn *) ret_ptr;

            D_DEBUG_AT( DirectFB_CoreLayerRegion, "=-> CoreLayerRegion_SetSurface\n" );

            ret = (DFBResult) CoreSurface_Lookup( core_dfb, args->surface_id, caller, &surface );
            if (ret) {
                 D_DERROR( ret, "%s(SetSurface): Looking up surface by ID %u failed!\n", __FUNCTION__, args->surface_id );
                 return_args->result = ret;
                 D_BREAK( "could not lookup object" );
                 return ret;
            }

            return_args->result = real.SetSurface( surface );
            if (return_args->result == DFB_OK) {
            }

            *ret_length = sizeof(CoreLayerRegionSetSurfaceReturn);
