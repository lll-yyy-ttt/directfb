                   ...
                   CoreWindow                 **ret_window )
{
     ...
     if (desc->flags & DWDESC_NAME)
     {
          if (strlen(desc->name) >= DFB_WINDOW_NAME_MAX_LENGTH)
          {
               D_DEBUG_AT( Core_Windows, "  -> CORE CREATE WINDOW NAME INVARG\n" );
               dfb_windowstack_unlock( stack );
               return DFB_INVARG;
          }

          strcpy(config.name, desc->name);
     }
