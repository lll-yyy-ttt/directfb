...
void IDirectFBWindow::SetName (const char *name)
{
     DFBCHECK( iface->SetName (iface, name) );
}

void IDirectFBWindow::GetName (char *buffer, int len)
{
     DFBCHECK( iface->GetName (iface, buffer, len) );
}

void IDirectFBWindow::SetHide (DFBBoolean hide)
{
     DFBCHECK( iface->SetHide (iface, hide) );
}

void IDirectFBWindow::GetHide (DFBBoolean *hide)
{
     DFBCHECK( iface->GetHide (iface, hide) );
}

void IDirectFBWindow::ScreenToClient (int *x, int *y)
{
     DFBCHECK( iface->ScreenToClient (iface, x, y) );
}
