             ...
             bool             right_eye )
{
     ...
     if ( SAWMAN_BORDER_TYPE_COLOR == border->type )
     {
          ...
          if (window->flags & CWF_FOCUSED) {
               colors      = border->color.focused;
               indices     = border->color.focused_index;
               num_colors  = D_ARRAY_SIZE(border->color.focused);
               num_indices = D_ARRAY_SIZE(border->color.focused_index);
          }
          else {
               colors      = border->color.unfocused;
               indices     = border->color.unfocused_index;
               num_colors  = D_ARRAY_SIZE(border->color.unfocused);
               num_indices = D_ARRAY_SIZE(border->color.unfocused_index);
          }
     ...
     else if ( SAWMAN_BORDER_TYPE_THEME == border->type )
     {
          /* Save clipping region. */
          old_clip = state->clip;
          ...
          // top
          rects[0].x = sawwin->bounds.x;
          rects[0].y = sawwin->bounds.y;
          rects[0].w = sawwin->bounds.w;
          rects[0].h = border->theme.top;
          ...
          if ( NULL != border->theme.theme_surface_top )
          {
               CoreSurface  *surface   = border->theme.theme_surface_top;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          {
               color = (DFBColor){ 0xff, 0x00, 0x00, 0xff };
               dfb_state_set_color( state, &color );
               CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
          }

          // bottom
          rects[0].x = sawwin->bounds.x;
          rects[0].y = sawwin->bounds.y + sawwin->bounds.h - border->theme.bottom;
          rects[0].w = sawwin->bounds.w;
          rects[0].h = border->theme.bottom;
          ...
          if ( NULL != border->theme.theme_surface_bottom )
          {
               CoreSurface  *surface   = border->theme.theme_surface_bottom;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          // left
          rects[0].x = sawwin->bounds.x;
          rects[0].y = sawwin->bounds.y + border->theme.top;
          rects[0].w = border->theme.left;
          rects[0].h = sawwin->bounds.h - border->theme.top - border->theme.bottom;
          ...
          if ( NULL != border->theme.theme_surface_left )
          {
               CoreSurface  *surface   = border->theme.theme_surface_left;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          // right
          rects[0].x = sawwin->bounds.x + sawwin->bounds.w - border->theme.right;
          rects[0].y = sawwin->bounds.y + border->theme.top;
          rects[0].w = border->theme.right;
          rects[0].h = sawwin->bounds.h - border->theme.top - border->theme.bottom;
          ...
          if ( NULL != border->theme.theme_surface_right )
          {
               CoreSurface  *surface   = border->theme.theme_surface_right;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          // close button
          rects[0] = border->theme.close_rect;
          rects[0].x += sawwin->bounds.x;
          rects[0].y += sawwin->bounds.y;
          ...
          if ( NULL != border->theme.close )
          {
               if (sawwin->close_focused)
               {
                    color = border->theme.close_select;
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }

               CoreSurface  *surface   = border->theme.close;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          else
          {
               if (sawwin->close_focused)
               {
                    color = (DFBColor){ 0xff, 0xc0, 0xc0, 0xc0 };
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }
               else
               {
                    color = (DFBColor){ 0xff, 0xff, 0x00, 0x00 };
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }
          }
          ...
          if ( NULL != border->theme.min )
          {
               if (sawwin->min_focused)
               {
                    color = border->theme.min_select;
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }

               CoreSurface  *surface   = border->theme.min;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          else
          {
               ...
               {
                    color = (DFBColor){ 0xff, 0xff, 0xff, 0x00 };
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }
          }
          ...
          if ( NULL != border->theme.max )
          {
               if (sawwin->max_focused)
               {
                    color = border->theme.max_select;
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }

               CoreSurface  *surface   = border->theme.max;
               DFBRectangle  src = { 0, 0, surface->config.size.w, surface->config.size.h };
               DFBRectangle  dst = rects[0];
          ...
          else
          {
               ...
               {
                    color = (DFBColor){ 0xff, 0x00, 0xff, 0x00 };
                    dfb_state_set_color( state, &color );
                    CoreGraphicsStateClient_FillRectangles( state->client, &rects[0], 1 );
               }
          }

          if (border->theme.font && '\0' != window->config.name[0])
          {
               dfb_state_set_color( state, &border->theme.title_color );
               SawMan_DrawString( state->client, border->theme.font,
                                  window->config.name, -1,
                                  rects[0].x + rects[0].w + 10, sawwin->bounds.y,
                                  DSTF_LEFT | DSTF_TOP, border->theme.encoding );
          }

#if 0
          if (window->flags & CWF_FOCUSED)
          {
               color = (DFBColor){ 0xff, 0x00, 0x90, 0xff };
               dfb_state_set_color( state, &color );
               rects[0] = sawwin->bounds;
               CoreGraphicsStateClient_DrawRectangles( state->client, &rects[0], 1 );
          }
          else
          {
               color = (DFBColor){ 0xff, 0x00, 0x65, 0xff };
               dfb_state_set_color( state, &color );
               rects[0] = sawwin->bounds;
               CoreGraphicsStateClient_DrawRectangles( state->client, &rects[0], 1 );
          }
                    ...
                    bool          right_eye )
{
     ...
     if (!input) {
          ...
          const SaWManBorderInit *winborder = &sawman_config->borders[sawman_window_priority(sawwin)];
          if ( SAWMAN_BORDER_TYPE_THEME == winborder->type &&
               sawman_window_border( sawwin ) )
          {
               if (dfb_region_intersect( region,
                                    sawwin->bounds.x + winborder->theme.left + offset,
                                    sawwin->bounds.y + winborder->theme.top,
                                    sawwin->bounds.x + sawwin->bounds.w - winborder->theme.right - 1 + offset,
                                    sawwin->bounds.y + sawwin->bounds.h - winborder->theme.bottom - 1 ))
               {
                    intersect = true;
               }
          }
