...
static DirectResult
ISaWManManager_SetHide( ISaWManManager     *thiz,
                        SaWManWindowHandle  handle,
                        DFBBoolean          hide )
{
     SaWMan       *sawman;
     ...
     return SaWManManager_SetWindowConfig( data->manager, sawwin, &config, SWMCF_HIDE );
}

static DirectResult
ISaWManManager_SetMaximization( ISaWManManager     *thiz,
                                SaWManWindowHandle  handle,
                                DFBBoolean          recover )
{
     SaWMan       *sawman;
     ...
     return sawman_window_maximization( window, recover );
}
...
static DirectResult
ISaWManManager_GetWindowName( ISaWManManager     *thiz,
                              SaWManWindowHandle  handle,
                              char               *buf,
                              int                 len )
{
     ...
     buf[0] = '\0';
     int l = strlen(sawwin->window->config.name);
     int n = (len-1) > l ? l : (len-1);
     strncpy(buf, sawwin->window->config.name, n);
     buf[n] = '\0';
...
static DirectResult
ISaWManManager_GetHide( ISaWManManager     *thiz,
                        SaWManWindowHandle  handle,
                        DFBBoolean         *ret_hide )
{
     SaWMan       *sawman;
     ...
     *ret_hide = sawwin->window->config.hide;

     return DFB_OK;
}

static DirectResult
ISaWManManager_GetMaximization( ISaWManManager     *thiz,
                                SaWManWindowHandle  handle,
                                DFBBoolean         *ret_max )
{
     SaWMan       *sawman;
     ...
     *ret_max = sawwin->is_maximization;

     return DFB_OK;
}

static DirectResult
ISaWManManager_RaiseToTop( ISaWManManager     *thiz,
                           SaWManWindowHandle  handle )
{
     SaWMan       *sawman;
     SaWManWindow *sawwin = (SaWManWindow*)handle;
     ...
     return SaWManManager_Restack( data->manager, sawwin, NULL, 1 );
}
                          ...
                          SaWManManager  *manager )
{
     ...
     thiz->SetHide         = ISaWManManager_SetHide;
     thiz->SetMaximization = ISaWManManager_SetMaximization;  
     ...
     thiz->GetWindowName   = ISaWManManager_GetWindowName;
     thiz->GetHide         = ISaWManManager_GetHide;
     thiz->GetMaximization = ISaWManManager_GetMaximization;
     thiz->RaiseToTop      = ISaWManManager_RaiseToTop;
