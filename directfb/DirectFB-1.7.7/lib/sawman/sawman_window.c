     ...
     if (region) {
          if ((update_flags & SWMUF_SCALE_REGION) && (window->config.options & DWOP_SCALE)) {
               ...
               if ((update_flags & SWMUF_UPDATE_BORDER) && sawman_window_border( sawwin ))
                    dfb_region_translate( &area, sawwin->bounds.x, sawwin->bounds.y );
               else
          ...
          {
               if ((update_flags & SWMUF_UPDATE_BORDER) && sawman_window_border( sawwin ))
                    area = DFB_REGION_INIT_TRANSLATED( region, sawwin->bounds.x, sawwin->bounds.y );
               else
                        ...
                        SaWManWindow *sawwin )
{
     ...
     {
          sawman_update_current_focus( sawman, sawwin, true );
     }
...
sawman_update_geometry( SaWManWindow *sawwin )
{
     ...
     /* adjust border. */
     sawman_adjust_window_bounds( sawwin, &sawwin->bounds );   
     ...
     {
          clip.x1 += border->theme.left;
          clip.y1 += border->theme.top;
          clip.x2 -= border->theme.right;
          clip.y2 -= border->theme.bottom;
     }
          ...
          sawman_window_border( sawwin ) )
     {
          ...
          // left
          region = DFB_REGION_INIT_FROM_RECTANGLE_VALS(0, 0, border->theme.left, sawwin->bounds.h );
          dfb_region_translate( &region, sawwin->window->config.z, 0 );
          sawman_update_window( sawman, sawwin, &region, DSFLIP_NONE, SWMUF_UPDATE_BORDER );
          dfb_region_translate( &region, -sawwin->window->config.z, 0 );
          if (stereo_layer)
          ...
          // right
          region = DFB_REGION_INIT_FROM_RECTANGLE_VALS(sawwin->bounds.w-border->theme.right, 0, border->theme.right, sawwin->bounds.h );
          dfb_region_translate( &region, sawwin->window->config.z, 0 );
          sawman_update_window( sawman, sawwin, &region, DSFLIP_NONE, SWMUF_UPDATE_BORDER );
          dfb_region_translate( &region, -sawwin->window->config.z, 0 );
          if (stereo_layer)
          ...
          // bottom
          region = DFB_REGION_INIT_FROM_RECTANGLE_VALS(0, sawwin->bounds.h-border->theme.bottom, sawwin->bounds.w, border->theme.bottom );
          dfb_region_translate( &region, sawwin->window->config.z, 0 );
          sawman_update_window( sawman, sawwin, &region, DSFLIP_NONE, SWMUF_UPDATE_BORDER );
          dfb_region_translate( &region, -sawwin->window->config.z, 0 );
          if (stereo_layer)
...
DirectResult
sawman_update_title( SaWManWindow *sawwin )
{
     CoreWindow   *window;
     SaWMan       *sawman;
     SaWManTier   *tier;
                 ...
                 DFBBoolean    hide )
{
     ...
     window = sawwin->window;
     old    = window->config.hide;
     tier   = sawman_tier_by_class(sawman, window->config.stacking);
     D_ASSERT(tier->region);

     if (old != hide) 
     {
          window->config.hide = hide;
          ...
          if (sawwin->flags & SWMWF_INSERTED) 
          {
               ...
               if (hide) 
                    D_DEBUG_AT( SaWMan_Focus, "  -> hide window... (focused %p)\n", sawman->focused_window );
               else
                    D_DEBUG_AT( SaWMan_Focus, "  -> show window... (focused %p)\n", sawman->focused_window );
          }
          else
...
static DirectResult
__sawman_set_opacity( SaWMan       *sawman,
                      SaWManWindow *sawwin,
                      u8            opacity,
                      bool          inhide )
{
     ...
     if (!inhide)
     {
          window->config.opacity_real = opacity;

          if (window->config.hide)
          {
               return DFB_OK;
          }
     }
                 ...
                 DFBBoolean    hide )
{
     ...
     if (old != hide) 
     {
          ...
          if (sawwin->flags & SWMWF_INSERTED) 
          {
               ...
               if (hide) 
               {
                    __sawman_set_opacity( sawman, sawwin, 0, true );

                    D_DEBUG_AT( SaWMan_Focus, "  -> hide window... (focused %p)\n", sawman->focused_window );
               }
               else
               {
                    __sawman_set_opacity( sawman, sawwin, window->config.opacity_real, true );
...
sawman_set_opacity( SaWMan       *sawman,
                    ...
                    u8            opacity )
{
     return __sawman_set_opacity( sawman, sawwin, opacity, false );
}
...
void
sawman_update_current_focus( SaWMan       *sawman,
                             SaWManWindow *sawwin,
                             bool          remove )
{
     D_MAGIC_ASSERT( sawman, SaWMan );
     ...
     if (remove)
     {
          ...
          while (--index >= 0)
          {
               swin = fusion_vector_at( &sawman->layout, index );
               ...
               if (!cwin->config.hide && cwin->config.opacity && 
                   !(cwin->config.options & DWOP_GHOST)) 
               {
     ...
     else
     {
          if (sawwin != sawman->focused_window)
          {
               sawman_switch_focus( sawman, sawwin, SWMCFR_IMPLICIT );
          }
     }
     ...
     else
     {
          ...
          fusion_vector_foreach_reverse (swin, index, sawman->layout) 
          {
               ...
               if (swin != sawwin && 
                   !cwin->config.hide && cwin->config.opacity && 
                   !(cwin->config.options & DWOP_GHOST)) 
               {
                    sawman_switch_focus( sawman, swin, SWMCFR_IMPLICIT );
                    break;
                          ...
                          int              y )
{
     ...
     if (!stack->cursor.enabled) {
          fusion_vector_foreach_reverse (sawwin, i, sawman->layout) {
               ...
               if (!window->config.hide && window->config.opacity && 
                   !(window->config.options & DWOP_GHOST))
                    return sawwin;
          }
     ...
     fusion_vector_foreach_reverse (sawwin, i, sawman->layout) {
          ...
          if (!(window->config.options & DWOP_GHOST) && 
              !window->config.hide && window->config.opacity &&
...
sawman_window_border( const SaWManWindow *sawwin )
{
     ...
     {
          int thickness = border->color.thickness;
