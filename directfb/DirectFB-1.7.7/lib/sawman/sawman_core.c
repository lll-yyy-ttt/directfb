             ...
             bool          copy_back )
{
     ...
     switch (call) {
          ...
          case SWMCID_WINDOW_CLOSE:
               if (!sawman->manager.callbacks.WindowClose)
                    return DFB_NOIMPL;
               break;

          case SWMCID_WINDOW_MIN:
               if (!sawman->manager.callbacks.WindowMin)
                    return DFB_NOIMPL;
               break;

          case SWMCID_WINDOW_MAX:
               if (!sawman->manager.callbacks.WindowMax)
                    return DFB_NOIMPL;
               break;
     }
                      ...
                      unsigned int *ret_length )
{
     ...
     switch (call) {
          ...
          case SWMCID_WINDOW_CLOSE:
               if (sawman->manager.callbacks.WindowClose)
                    *ret_val = sawman->manager.callbacks.WindowClose( sawman->manager.context, call_ptr );
               break;

          case SWMCID_WINDOW_MIN:
               if (sawman->manager.callbacks.WindowMin)
                    *ret_val = sawman->manager.callbacks.WindowMin( sawman->manager.context, call_ptr );
               break;

          case SWMCID_WINDOW_MAX:
               if (sawman->manager.callbacks.WindowMax)
                    *ret_val = sawman->manager.callbacks.WindowMax( sawman->manager.context, call_ptr );
               break;
