...
struct vt_line *vt_newline(struct vt_em *vt, int len);
...
vt_scrollback_add(struct vt_em *vt, struct vt_line *wn)
{
     ...
     ln = malloc(VT_LINE_SIZE(wn->width));
     ln->next = NULL;
     ln->prev = NULL;
     ln->wrap = wn->wrap;
     ln->width = wn->width;
...
vt_scroll_up(struct vt_em *vt, int count)
{
     ...
     while (count>0) {
          ...
          if (!wn) {
               fprintf(stderr, "vt_scroll_up: could not find line %d\n", vt->scrolltop);
          }
...
/* line feed (most probably from line wrap) */
static void vt_lf(struct vt_em *vt)
{
     ...
     wrap = vt->this_line->wrap;
     if (!wrap++) {
          if (vt->this_line->prev && !vt->this_line->prev->wrap)
               wrap++;
          
          vt->this_line->wrap = wrap++;
     }

     if (vt->cursory != vt->scrollbottom && vt->cursory < vt->height - 1) {
          vt->cursory++;
          d(printf("new ypos = %d\n", vt->cursory));
...
/* real line feed (from control character) */
static void vt_rlf(struct vt_em *vt)
{
    d(printf("lf (real)\n"));
...
vt_dsr(struct vt_em *vt)
{
     ...
     switch (vt->arg.num.intargs[0]) {
          case 5:           /* report 'ok' status */
               snprintf(status, sizeof(status), "\033[0n");
               break;
          case 6:           /* report cursor position */
               snprintf(status, sizeof(status), "\033[%d;%dR",
...
struct vt_jump vtjumps[] = {
     ...
     {vt_backspace,VT_CON}, {vt_tab,VT_CON}, {vt_rlf,VT_CON}, {0,0}, /* 8: ^H ^K */
...
vt_resize_lines(struct vt_line *wn, int width, uint32 default_attr)
{
     ...
     while (nn) {
          ...
          if (wn->width < width) {
               ...
               /* resize the line */
               wn = realloc(wn, VT_LINE_SIZE(width));
     ...
     if (height < vt->height) {
          ...
          for (i = 0; i < count; i++) {
               ...
               if (vt->scrollbacklines > 0) {
                    ...
                    nn = vt_newline(vt, MAX(vt->width, wn->width));
                    ...
                    vt_list_addhead(&vt->lines_alt, (struct vt_listnode *)vt_newline(vt,0));
                    vt_list_addhead(&vt->lines_back, (struct vt_listnode *)vt_newline(vt,0));
                    ...
                    vt_list_addtail(&vt->lines, (struct vt_listnode *)vt_newline(vt,0));
                    vt_list_addtail(&vt->lines_back, (struct vt_listnode *)vt_newline(vt,0));
                    vt_list_addtail(&vt->lines_alt, (struct vt_listnode *)vt_newline(vt,0));
               } /* if scrollbacklines */
          }
     ...
     if (y>=0) {
          ...
          switch (mode) {
               case VTMODE_SEND_MOUSE_PRESS: /* press only */
                    if (down)
                         snprintf(mouse_info, sizeof(mouse_info), "\033[M%c%c%c",
                                  ' ' + ((button - 1)&3),
                                  x+' '+1,
                                  y+' '+1);
                    break;
