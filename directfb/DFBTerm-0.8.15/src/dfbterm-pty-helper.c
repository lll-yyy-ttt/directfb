/*
 * dfbterm-pty.c:  Helper setuid application used to open a pseudo-
...
#include <utmp.h>
#include <grp.h>
#include "dfbterm-pty.h"
#include "dfbterm-login-support.h"
...
init_term_with_defaults(struct termios* term) {
      ...
      * but we use the defaults, with erase bound to the key dfbterm-terminal
      * maps.
      *
...
#error "No means to raise privileges! Huge security risk! Won't compile."
     ...
     fchown (slave_pty, getuid (), group_info ? group_info->gr_gid : (gid_t)-1);
...
sanity_checks (void)
{
     ...
     for (i = 0; sensible_limits [i].value != -1; i++) {
          ...
          if (rlim.rlim_cur != RLIM_INFINITY &&
              (int)rlim.rlim_cur < sensible_limits [i].value) {
...
main (int argc, char *argv [])
{
     ...
     for (;;) {
          ...
          switch (op) {
               case DFBTERM_PTY_OPEN_PTY_UTMP:
                    open_ptys (1, 0, 0);
                    break;

               case DFBTERM_PTY_OPEN_PTY_UWTMP:
                    open_ptys (1, 1, 0);
                    break;

               case DFBTERM_PTY_OPEN_PTY_WTMP:
                    open_ptys (0, 1, 0);
                    break;

               case DFBTERM_PTY_OPEN_PTY_LASTLOG:
                    open_ptys (0, 0, 1);
                    break;

               case DFBTERM_PTY_OPEN_PTY_LASTLOGUTMP:
                    open_ptys (1, 0, 1);
                    break;

               case DFBTERM_PTY_OPEN_PTY_LASTLOGUWTMP:
                    open_ptys (1, 1, 1);
                    break;

               case DFBTERM_PTY_OPEN_PTY_LASTLOGWTMP:
                    open_ptys (0, 1, 1);
                    break;

               case DFBTERM_PTY_OPEN_NO_DB_UPDATE:
                    open_ptys (0, 0, 0);
                    break;

               case DFBTERM_PTY_RESET_TO_DEFAULTS:
                    break;

               case DFBTERM_PTY_CLOSE_PTY:
